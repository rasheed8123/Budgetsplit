...
router.get('/:id/balances', auth, async (req, res) => {
  try {
    ...
    const exchangeRates = getExchangeRates(); // optional if not needed by convertCurrency()
    ...
    expenses.forEach(expense => {
      ...
      // Convert total expense amount to default currency
      const expenseAmountInDefault = convertCurrency(
        expense.amount,
        expenseCurrency,
        defaultCurrency
      );
      ...
      if (balances[paidById]) {
        balances[paidById].paid += expenseAmountInDefault;
      }
      ...
      expense.participants.forEach(participant => {
        ...
        // Convert participant's share to default currency
        const shareInDefault = convertCurrency(
          share,
          expenseCurrency,
          defaultCurrency
        );
        ...
        if (balances[participantId] && participantId !== paidById) {
          balances[participantId].owes += shareInDefault;
        }
      });
    });
    ...
    const result = membersDetails.map(member => ({
      ...
      paid: parseFloat(balances[member._id.toString()].paid.toFixed(2)),
      owes: parseFloat(balances[member._id.toString()].owes.toFixed(2)),
      netBalance: parseFloat(balances[member._id.toString()].netBalance.toFixed(2)),

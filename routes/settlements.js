  ...
  try {
    ...
    expenses.forEach(expense => {
      ...
      expense.participants.forEach(participant => {
        ...
        console.log("shareInDefaultCurrency",shareInDefaultCurrency)
        // Initialize balances
...
Object.entries(netBalances).forEach(([debtorId, creditorMap]) => {
  Object.entries(creditorMap).forEach(([creditorId, amount]) => {
    if (!creditorTotals[creditorId]) {
      creditorTotals[creditorId] = 0;
    }
    creditorTotals[creditorId] += amount;
  });
});

const creditors = Object.entries(creditorTotals)
  .map(([id, totalOwed]) => ({
    id,
    totalOwed,
    owedBy: {} // You can expand this if needed
  }))
  .sort((a, b) => b.totalOwed - a.totalOwed); // Sort by descending credit
    ...
    while (debtors.length > 0 && creditors.length > 0) {
      ...
      console.log("debtorsfinal",debtor)
